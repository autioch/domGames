...
(function(q) {
  ...
  function qbGlobesArea(qbContainer) {
    ...
    qb.onResume = function() {
      ...
      qb.refreshTargets();
      GC.options.pause.pressed(false).action(() => {
        GC.game.pause(true);
      });
    };
    qb.onStop = function() {
  ...
  function initializePopup() {
    ...
    GC.options.parseCss(config.size.options).parseConfig(config.options).addBorders(false, true, true)
      .show();
    GC.options.sound.action(toggleSound).pressed(!q.sound);
    GC.options.pause.action(() => {
      GC.game.pause(true);
    });
    GC.options.exit.action(gameExit);
    GC.stats = new q.qbStatistics(GC);
    GC.stats.parseCss(config.size.stats).parseConfig(config.stats).addBorders(false, true)
      .show();
  ...
  function startSplash() {
    ...
    tsb = 1;
    GC.stats.val('active', 0).val('hit', 0).val('points', 0)
      .val('life', gamec.life)
      .val('speed', (gamec.freq / 1000).toFixed(2));
