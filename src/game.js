...
export default function game(locale, difficulty) {
  ...
  function gamePrepare() {
    ...
    // koniec gry oznacza też pauze :D
    ...
    statistics.val('active', 0).val('hit', 0).val('points', 0)
      .val('life', setup.life)
      .val('speed', (gamearea.freq / 1000).toFixed(2));
  ...
  function speedUp() {
    ...
    gamearea.tsb += Math.round(4500 / gamearea.freq);
  ...
  function addTarget() {
    if (!gamearea.paused) {
      ...
      const t = new Target(gamearea.div, r + setup.msize, setup.age);

      gamearea.targets.push(t);
      t.on('hit', targetHit).on('age', targetAge).locateRandom()
        .present();
  ...
  function setWallpaper() {
    ...
    prev.div.fadeOut(1000, (index, el) => $(el).css(bgi, `url('${config.wallpaper.location}${img}.jpg')`)); // eslint-disable-line no-unused-vars
