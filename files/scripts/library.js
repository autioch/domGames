...
(function(q) {
  ...
  q.install = function(location, log, success, error, notice) {
    if (window.navigator.mozApps) {
      log.click(() => {
        const request = window.navigator.mozApps.install(location);
  ...
  function b(qbc) {
    ...
    x.present = function(speed, callback) {
      ...
      switch (typeof speed) {
        case 'undefined':c = undefined;
          break;
        case 'function':c = speed;
          break;
        default:x.presentSpeed = speed;
          c = callback;
      }
  ...
  function qbBorderVertical(qbc, top, left, height) {
    b.call(this, qbc);
    this.parseCss({
      top,
      left,
  ...
  function qbBorderHorizontal(qbc, top, left, width) {
    b.call(this, qbc);
    this.parseCss({
      top,
      left,
  ...
  function qbStatField(qbc, id, desc, title, max, val) {
    ...
    x.desc(desc || '').title(title || '').val(val || 0)
      .max(max || 0).div.attr('id', `qbStat${id}`);
  }
  ...
  function qbSplashAlert(qbc) {
    ...
    x.splash = function(message, color, bg) {
      x.div.stop(true, true).show(0).css({
        color: color || '#f00',
  ...
  function qbOptions(qbc) {
    ...
    x.parseConfig = function(obj) {
      ...
      for (item in obj) {
        a = obj[item];
        switch (a.type) {
          case 'space':x.addSpace(item, a.height);
            break;
          case 'title':x.addTitle(item, a.desc);
            break;
          case 'button':x.addButton(item, a.desc, a.title);
            break;
          case 'border':x.addBorder(item);
            break;
          default:
        }
        if (a.visible) {
 ...
 expires=${date.toGMTString()}`;
  ...
  try {
    ...
    console.log(err.message);
    q.cache = qbCookieStorage;
  }
  q.qbBackground = qbBackground;
  q.qbClock = qbClock;
  q.qbGameArea = qbGameArea;
  q.qbConfirmMessage = qbConfirmMessage;
